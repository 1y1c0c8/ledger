...
- `src/Index.html` — 前端 UI（HtmlService）。三個分頁：
  ...
  - **帳戶**：淨資產 ＋ 各帳戶餘額（跨月累計）。信用卡顯示**本期未繳**（有設結帳日才算；否則退回累計未繳）＋結帳/繳款日＋上期待繳提醒；**點卡片 → 卡片帳單浮層**（切換帳單週期、明細、勾選計入本期/延期、我/家分攤、設定結帳/繳款日、`記錄繳款`、`撤銷繳款`）。
  - 待辦：「本月卡費分攤」(跨卡加總)區塊已移除，待重做——已知問題：(1) 沒按各筆實際入帳日歸期（入帳日不固定）；(2) 多張卡加總在一起看太亂，應改成分卡。
...
- **帳單月**(J欄)：信用卡延期標記，`yyyy-MM`=把這筆延到該結帳月、空白=照消費日歸期。後端用 `CREATED_COL=8`、`BILL_COL=10` 固定抓欄位（加欄後不可用 `TXN_HEADERS.length`）。**雷**：`'yyyy-MM'` 寫進儲存格會被試算表自動當成日期，所以 `setChargeBill` 用 `setNumberFormat('@')` 存純文字、讀取一律走 `billStr_()`(遇 Date 還原成 `yyyy-MM`)。
